		...
		while (slotTime < this.maxTime) {
			...
			axisHtml =
				'<td class="fc-axis fc-time ' + view.widgetContentClass + '" ' + view.axisStyleAttr() + '>' +
					((!minutes) ? // if irregular slot duration, or on the hour, then display the time
...
$.extend(TimeGrid.prototype, {
	...
	generateSegPositionCss: function(seg) {
		...
		//Added by Josef Meile: placeholders will be rendered with a
		//transparent color, they will occupy the whole table cell,
		//mouse events will be forwarded to the other events, and
		//they won't overlap other events
		if (seg.event.placeholder){
			left = 0;
			right = 0;
			props.width = "100%";
		...
		if (seg.event.annotation){
			props.opacity = view.opt('annotationOpacity');
		}
		if ("class_full" in seg.event){
			if (seg.event.class_full){
				props.border = "1px solid #d9534f";
				props["box-shadow"] = "0px 0px 3px #d9534f";
				props["border-radius"] = "7px";
			} else {
				props.border = "1px solid #5cb85c";
				props["box-shadow"] = "0px 0px 3px #5cb85c";
				props["border-radius"] = "7px";
			}
		}
		...
		//the placeholders, they will always occupy the whole table
		//cell
		if (!seg.event.placeholder){
...
function buildSlotSegLevels(segs) {
	...
	for (i=0; i<segs.length; i++) {
		...
		//placeholders will be always assigned to level 0
		if (seg.placeholder)
		{
			j = 0;
			seg.level = 0;
...
function computeSlotSegCollisions(seg, otherSegs, results) {
	...
	for (var i=0; i<otherSegs.length; i++) {
		//Added by Josef Meile: placeholder won't collide with other events
		if (!otherSegs[i].event.placeholder) {
